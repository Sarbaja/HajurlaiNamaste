<?php
    ...
    if(isset($_POST['createservices'])){
        ...
        if ($_FILES["file"]["size"] > 0 && $_FILES["file"]["size"] < 2097152){
            $sql= "INSERT INTO services(title, content, image) VALUES ('$title', '$content', '$image')";
            $result = $connection->query($sql);

            if($result){
                header("Location:services.php?success=" . urlencode("Services added successfully!!"));
            }
            else{
              header("Location:services.php?error=" . urlencode("Services cannot be created!!"));
            }
        }
        else{
                ...
                <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>" method="POST" enctype="multipart/form-data">
                  ...
                  <div class="row">
                    <div class="update ml-auto mr-auto">
                      <button type="submit" class="btn btn-primary btn-round" name="createservices">Add Services</button>
                    </div>
                  </div>
                </form>
              </div>
