<?php
    ...
    if(isset($_POST['updateservices'])){
        ...
        $file_store = "uploads/services/" .$file_name;
        ...
        if ($_FILES["file"]["size"] > 0 && $_FILES["file"]["size"] < 2097152){
            $sql= "UPDATE services SET title = '$title', content = '$content', image = '$image' WHERE id= $id";
            $result = $connection->query($sql);

            if($result){
                header("Location:services.php?success=" . urlencode("Services updated successfully!!"));
            }
            else{
              header("Location:services.php?error=" . urlencode("Services cannot be updated!!"));
            }
        }
        else{
            header("Location:services.php?error=" . urlencode("Services upload failed!! Image file size must be less than 2MB!!"));
        }
    }
                ...
                <form action="" method="POST" enctype="multipart/form-data">
                  ...
                  <div class="row">
                    <div class="update ml-auto mr-auto">
                      <button type="submit" class="btn btn-primary btn-round" name="updateservices">Update Services</button>
                    </div>
                  </div>
                </form>
              </div>
