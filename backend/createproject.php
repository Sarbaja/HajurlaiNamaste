<?php
    ...
    if(isset($_POST['createproject'])){
        ...
        if ($_FILES["file"]["size"] > 0 && $_FILES["file"]["size"] < 2097152){
            $sql= "INSERT INTO projects(name, content, image) VALUES ('$name', '$content', '$image')";
            $result = $connection->query($sql);

            if($result){
                header("Location:projects.php?success=" . urlencode("Projects added successfully!!"));
            }
            else{
              header("Location:projects.php?error=" . urlencode("Projects cannot be created!!"));
            }
                ...
                <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>" method="POST" enctype="multipart/form-data">
                  ...
                  <div class="row">
                    <div class="update ml-auto mr-auto">
                      <button type="submit" class="btn btn-primary btn-round" name="createproject">Create Project</button>
                    </div>
                  </div>
                </form>
              </div>
