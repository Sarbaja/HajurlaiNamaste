<?php
    ...
    if(isset($_POST['createclient'])){
        ...
        if ($_FILES["file"]["size"] > 0 && $_FILES["file"]["size"] < 2097152){
            $sql= "INSERT INTO clients(name, logo) VALUES ('$title', '$image')";
            $result = $connection->query($sql);

            if($result){
                header("Location:clients.php?success=" . urlencode("Clients logo added successfully!!"));
            }
            else{
              header("Location:clients.php?error=" . urlencode("Clients logo cannot be created!!"));
            }
        }
        else{
                ...
                <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>" method="POST" enctype="multipart/form-data">
                  ...
                  <div class="row">
                    <div class="update ml-auto mr-auto">
                      <button type="submit" class="btn btn-primary btn-round" name="createclient">Add Client's Logo</button>
                    </div>
                  </div>
                </form>
              </div>
